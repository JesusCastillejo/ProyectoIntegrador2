...
const Alta = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.nombre === '' || form.nombre.length < 2 || form.nombre.length > 25 || !/^[\u00C0-\u00FFa-zA-Z\s,.\-¿?¡!]+$/.test(form.nombre)) {
      toast.info('El campo nombre no puede estar vacio y debe contener entre 2 y 15 caracteres.')
      return;
    }


    if (form.stock === '' || isNaN(form.stock) || form.stock < 1 || form.stock > 1000) {
      toast.info('El campo stock solo debe aceptar números entre 1 y 1000.');
      return;
    }
    ...
    if (!/^[\u00C0-\u00FFa-zA-Z\s,.\-¿?¡!]+$/.test(form.descripcion) || form.descripcion.length < 3 || form.descripcion.length > 30) {
      toast.info('El campo descripcion no debe estar vacio, debe agregar una descripcion corta entre 5 y 30 caracteres.');
      return;
    }


    if (!/^[\u00C0-\u00FFa-zA-Z\s,.\-¿?¡!]+$/.test(form.descripcionlarga) || form.descripcionlarga.length < 5 || form.descripcionlarga.length > 1000) {
      toast.info('el campo descripcion larga no debe estar vacio, debe de agregar una descripcion larga entre 5 y 1000 caracteres.');
      return;
    }
    ...
    if (!form.foto) {
      toast.info('EL campo foto debe tener un archivo valido.');
      return;
